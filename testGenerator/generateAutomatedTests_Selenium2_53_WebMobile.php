...
require 'generateAutomatedTests_Selenium2_53_WebMobile.conf.php';
	...
	public function generateTestCase () {
	   ...
	   switch ($osvalue) {
	    ...
	    self::writeLine($id_testcaseFile, "		selenium.setTimeout(Config.window_attach_timeout);");
 	    self::writeLine($id_testcaseFile, '		driver.get("'.$urlvalue.'");');
		   ...
		   switch ($osvalue) {
			...
			foreach ($tests as $test) {
				...
				if ($index_scenario>$index_start) {
					...
					switch ($browser) {
						case 'Chrome' : 
							self::writeLine($id_testcaseFile, '		System.out.println(">> driver.quit");
							driver.quit();
