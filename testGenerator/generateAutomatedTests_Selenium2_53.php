...
require 'generateAutomatedTests_Selenium2_53.conf.php';
...
class generateAutomatedTests_Selenium2_53 implements automatedTestsGenerator {
	...
	*/
	...
	public function generateTestCase () {
	    ...
	    $urlvalue= $array_urlvalue[1];
	  	...
	  	//instantiate driver according to browser
	    switch ($browser) {
	    	case 'Internet Explorer': 
	    		self::writeLine($id_testcaseFile, "		File file = new File(Config.dir_config + \"/IEDriverServer.exe\");");
	    		self::writeLine($id_testcaseFile, "		System.setProperty(\"webdriver.ie.driver\", file.getAbsolutePath());");
	    		self::writeLine($id_testcaseFile, "		WebDriver driver = new InternetExplorerDriver();");
	    		break;
	    	case 'Firefox' : 
	    		self::writeLine($id_testcaseFile, "		WebDriver driver = new FirefoxDriver();");
	    		break;
	    	case 'Chrome':
	    		self::writeLine($id_testcaseFile, "		File file = new File(Config.dir_config + \"/chromedriver.exe\");");
	    		self::writeLine($id_testcaseFile, "		System.setProperty(\"webdriver.chrome.driver\", file.getAbsolutePath());");	    		 
	    		self::writeLine($id_testcaseFile, "		WebDriver driver = new ChromeDriver();");
	    		break;
	    		
	    	case 'Safari' : 
	    		self::writeLine($id_testcaseFile, "		WebDriver driver = new OperaDriver();");
	    		break;
	    		
	    	case 'Opera' : 
	    		self::writeLine($id_testcaseFile, "		WebDriver driver = new SafariDriver();");
	    		break;
	    	
	    }
	   
	    self::writeLine($id_testcaseFile, '		'.$testcase_name.'.selfdriver = driver;');
	   
	    
	    //implement Selenium driver using url
	    self::writeLine($id_testcaseFile, "		//Create the Selenium implementation");
	    self::writeLine($id_testcaseFile, "		WebDriverBackedSelenium selenium = new WebDriverBackedSelenium(driver, \"".$urlvalue."\");");
	    self::writeLine($id_testcaseFile, "		selenium.setTimeout(Config.window_attach_timeout);");
	    
	    if ($urlvalue!="") {
	    	self::writeLine($id_testcaseFile, "		selenium.open(\"".$urlvalue."\");");
	    	self::writeLine($id_testcaseFile, "		selenium.windowMaximize();");
	    }
		...
		foreach ($tests as $test) {
	 		...
	 		if ($index_scenario>$index_start) {
		 		...
		 		//kill driver in case of execution error
		 		switch ($browser) {
		 			case 'Internet Explorer':
		 				self::writeLine($id_testcaseFile, "			Runtime rt = Runtime.getRuntime();");
		 				self::writeLine($id_testcaseFile, "			rt.exec(\"taskkill /F /IM IEDriverServer.exe\");");
		 				break;
		 			case 'Chrome':
		 				self::writeLine($id_testcaseFile, "			Runtime rt = Runtime.getRuntime();");
		 				self::writeLine($id_testcaseFile, "			rt.exec(\"taskkill /F /IM chromedriver.exe\");");
		 				break;
		 		}
	...
	public function generateScenariosFile() {
		...
		foreach ($tests as $test) {
			self::writeLine($id_scenariosFile, 'public static boolean '.traiterAccents ($test->getScenarioName()).'_'.$test->getScenarioId().' (WebDriverBackedSelenium selenium, int testcase_id, String testcase_label, int scenario_id, String scenario_label, int scenario_index) throws Exception {');
